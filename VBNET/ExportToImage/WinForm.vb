' =============================================================================
' ExportToImage sample - VB.NET ActiveX (XDK11)
'
' Demonstrates how to export a GIS map layer to a raster image or a numeric
' elevation grid using the TatukGIS Extended Developer Kernel (XDK11) ActiveX API.
'
' This variant targets the COM/ActiveX binding (TatukGIS_XDK11) rather than the
' pure-managed NDK assembly used by the WinForms variant. The key difference is
' that the viewer is an AxTGIS_ViewerWnd ActiveX host, and collection access uses
' the COM Items.item(index) call rather than the .NET indexer. The application
' logic (resolution presets, format capabilities, ImportLayer) is identical.
'
' Key concepts shown:
'   - Hosting the ActiveX TGIS_ViewerWnd (AxTGIS_ViewerWnd) in a WinForms form.
'   - Creating a new output TGIS_LayerPixel via TGIS_Utils.GisCreateLayer.
'   - Querying lpx.Capabilities (TGIS_LayerPixelSubFormatList) to enumerate
'     the sub-formats (pixel depth, compression) supported by the target format.
'   - Three resolution presets: Best quality, For document (300 DPI), For Web (96 DPI).
'   - Two extent options: full geographic bounding box, or current viewport.
'   - Performing the raster conversion with TGIS_LayerPixel.ImportLayer_3.
' =============================================================================
...
Namespace ExportToImage
    ''' <summary>
    ''' Main application form for the ExportToImage ActiveX sample.
    ...
    Public Class WinForm
        ...
        ''' Created by GisCreateLayer and populated by ImportLayer_3.</summary>
        ...
        ''' <summary>Output image dimensions in pixels. Passed to ImportLayer_3.
        ...
        ''' <summary>ActiveX-hosted TatukGIS viewer. Declared as Friend so the
        ''' designer-generated code can wire events via the COM event sink.</summary>
...
#Region "Windows Form Designer generated code"
        ...
        Private Sub InitializeComponent()
            ...
            Me.rbGrid.Location = New System.Drawing.Point(449, 59)
            ...
            Me.rbImage.Location = New System.Drawing.Point(449, 35)
    ...
    "elStore (*.ttkps)|*.ttkps"
            '
            'GIS - ActiveX host for AxTGIS_ViewerWnd; OcxState restores persisted control state
            '
            Me.GIS.Enabled = True
            Me.GIS.Location = New System.Drawing.Point(23, 29)
            Me.GIS.Name = "GIS"
            Me.GIS.OcxState = CType(resources.GetObject("GIS.OcxState"), System.Windows.Forms.AxHost.State)
            Me.GIS.Size = New System.Drawing.Size(409, 186)
            Me.GIS.TabIndex = 5
            '
            'WinForm
            '
            ...
            Me.ClientSize = New System.Drawing.Size(565, 508)
...
#End Region
        ...
        ''' <summary>
        ''' Instance of TGIS_Utils used to resolve the sample data directory and to
        ''' call GisCreateLayer. In the ActiveX binding, TGIS_Utils is instantiated
        ''' as an object rather than accessed via shared/static methods.
        ''' </summary>
        Dim GisUtils As New TGIS_Utils()

        ''' <summary>
        ''' Application entry point. Configures visual styles before launching the form.
        ''' </summary>
        <STAThread>
        ...
        ''' controls. The hdr.adf is an ESRI Arc/Info Binary Grid header;DKopens
        ...
        Private Sub rbGrid_CheckedChanged(sender As Object, e As EventArgs) Handles rbGrid.CheckedChanged
            GIS.Open(GisUtils.GisSamplesDataDirDownload() + "\World\Countries\USA\States\California\San Bernardino\NED\hdr.adf")
            tbPath.Clear()
            cbFormat.ResetText()
            cbFormat.Items.Clear()
            groupBox3.Enabled = False
        ...
        '''
        ''' Note: this variant calls ImportLayer_3 (the COM-dispatchable overload that
        ''' accepts a TGIS_LayerPixelSubFormat parameter) rather than the managed
        ''' ImportLayer overload used in the NDK WinForms variant.
        ''' </summary>
        Private Sub btnExport_Click(sender As Object, e As EventArgs) Handles btnExport.Click
            ...
            ' ImportLayer_3 is the COM-dispatchable overload that accepts a
            ' TGIS_LayerPixelSubFormat struct as the final argument. It resamples
            ' lstp (source) into lpx (output file) at the requested pixel size.
            lpx.ImportLayer_3(lstp, FExtent, lstp.CS, CUInt(PixWidth), CUInt(PixHeight), c)
            MessageBox.Show("Done!", "File exported")
        End Sub

        ''' <summary>
        ''' Open the save-file dialog, create the output pixel layer via GisCreateLayer,
        ''' and populate the format combobox with supported sub-formats.
        '''
        ''' Note: in the ActiveX binding, layer collection items are accessed via the
        ''' COM Items.item(index) method rather than a .NET indexer, and the
        ''' Capabilities collection is a TGIS_LayerPixelSubFormatList (COM collection)
        ''' iterated with an integer index loop.
        ''' </summary>
        Private Sub btnOpen_Click(sender As Object, e As EventArgs) Handles btnOpen.Click
            Dim clst As TGIS_LayerPixelSubFormatList
            Dim c As TGIS_LayerPixelSubFormat
            Dim i As Integer
            ...
            ' Items.item(0) accesses the bottom-most (primary) layer via the COM collection
            lstp = DirectCast(GIS.Items.item(0), TGIS_LayerPixel)

            ' GisCreateLayer (via the GisUtils instance) selects the correctDKlayer
            ' driver from the file extension and returns an empty TGIS_LayerPixel.
            If rbImage.Checked Then
                lpx = TryCast(GisUtils.GisCreateLayer(GetFileName(tbPath.Text), dlgSaveImage.FileName), TGIS_LayerPixel)
            Else
                lpx = TryCast(GisUtils.GisCreateLayer(GetFileName(tbPath.Text), dlgSaveGrid.FileName), TGIS_LayerPixel)
            End If

            ' Capabilities returns all valid TGIS_LayerPixelSubFormat options.
            ' Iterate by index (COM-style) and build the combobox with deep copies.
            clst = lpx.Capabilities
            i = 0
            caps = New T_capability(clst.Count) {}
            For i = 0 To clst.count - 1
                c = clst.Item(i)
        ...
        ''' Falls back to DEFAULT_WIDTHPIX (4200 px) when no raster layer is loaded.
        ''' </summary>
        Private Sub ValuesInit()
            ...
            ' Note: Items.Item(i) is used here (COM collection access).
            For i = GIS.Items.Count - 1 To 1 Step -1
                la = DirectCast(GIS.Items.Item(i), TGIS_Layer)
            ...
            If density = 0 Then
                ...
                widthpix = 4200
            Else
                la = DirectCast(GIS.Items.Item(j), TGIS_Layer)
                ext_width = la.Extent.XMax - la.Extent.XMin
                ...
                widthpix = CInt(Math.Round(ext_delta * DirectCast(GIS.Items.Item(j), TGIS_LayerPixel).BitWidth))
            End If
