...
Namespace LayerStatistics
    Public Class Form1
        ...
        Private Sub InitializeComponent()
            ...
            Me.rtbResult.Size = New System.Drawing.Size(261, 648)
        ...
        Dim GisStatisticalFunctions As New TGIS_StatisticalFunctions()

        Dim GisStatisticsFactory As New TGIS_StatisticsFactory()
        ...
        Private Sub WinForm_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
            ...
            openDialog.Filter = GisUtils.GisSupportedFiles(TGIS_FileType.Grid Or TGIS_FileType.Pixel Or TGIS_FileType.Vector, False)
            sample_dir = GisUtils.GisSamplesDataDirDownload() + "/World/Countries/USA/States/California"
        ...
        Private Function prepareFunctions(<Out> ByRef _funcs As TGIS_StatisticalFunctions) As Boolean
            Dim res As Boolean
            res = False
            _funcs = GisStatisticalFunctions.EmptyStatistics()
        ...
        Private Sub btnCalculate_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnCalculate.Click
            ...
            Try
                ...
                ll = CType(GIS.Items.Item(0), TGIS_Layer)

                ll.Statistics.UseBesselCorrection = cbBessel.Checked

                For i As Integer = 0 To cblDefs.Items.Count - 1
                    If cblDefs.GetItemChecked(i) Then ll.Statistics.Add_2(cblDefs.Items(i).ToString(), funcs)
                Next
                ...
                ll.Statistics.Calculate_2(GIS.VisibleExtent, Nothing, "", cbFastStats.Checked)
        ...
        Private Sub showResults(ByVal _stats_engine As TGIS_StatisticsAbstract, clear As Boolean)
            ...
            Dim stats_available As TGIS_StatisticsItemList
            ...
            For i = 0 To _stats_engine.AvailableResults.Count - 1
                ...
                rtbResult.AppendText(_stats_engine.AvailableResults.Strings(i) & Environment.NewLine)
                rtbResult.AppendText(DASHED_LINE & Environment.NewLine)
                stats_result = _stats_engine.Get(_stats_engine.AvailableResults.Strings(i))
        ...
        Private Sub doBusyEvent(Pos As Integer, [End] As Integer, ByRef Abort As Boolean) Handles GIS.BusyEvent
            If Pos < 0 Then
                pProgress.Value = 0
            ElseIf Pos = 0 Then
                pProgress.Minimum = 0
