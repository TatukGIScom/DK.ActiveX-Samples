...
Namespace SQLWizard
    ...
    Public Class SQLForm
        ...
        Private GIS_TMP As AxTatukGIS_XDK11.AxTGIS_ViewerWnd

        Private imgList As ImageList

        Private lst As TGIS_StringList
        ...
        Public Sub setGIS(ByVal _gis As AxTatukGIS_XDK11.AxTGIS_ViewerWnd)
            GIS_TMP = _gis
        End Sub
...
#Region "Windows Form Designer generated code"
        ...
        Private Sub InitializeComponent()
            ...
            Me.dlgOpen.Filter = "SQLLite|*.sqlite|All files|*.*"
            '
            'dlgSave
            '
...
#End Region
        ...
        Private Sub SQLForm_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
            ...
            cbxStorage.Items.Add("ORACLESPATIAL_PC")
            cbxStorage.Items.Add("ORACLESPATIAL_TIN")
        ...
        Private Function prepareConnectString(ByVal _txt As String) As TGIS_StringList
            Dim tkn As TGIS_Tokenizer
            Dim c As New TGIS_CharArray
            Dim finalResult As TGIS_StringList
            tkn = New TGIS_Tokenizer
            c.Value(0) = 59
            tkn.ExecuteEx_2(_txt, c.Value(0))
            finalResult = New TGIS_StringList
            finalResult.AddStrings(tkn.Result)
            Return finalResult
        End Function

        Private Function prepareOCI() As TGIS_StringList
            Dim str As TGIS_StringList
            Dim finalResult As TGIS_StringList
            finalResult = New TGIS_StringList
            str = prepareConnectString(cbxConnString.Text)
            finalResult.Add("USER" + ("=" + str.Values("User ID")))
            finalResult.Add("USER" + (" = " + str.Values("User ID")))
            ...
            Do While (i _
                ...
                If ((tkn.Result.Names(i) = "Integrated Security") _
                    ...
                    finalResult = (finalResult _
                                + (tkn.Result.Strings(i) + Microsoft.VisualBasic.ChrW(59)))
                End If
        ...
        Private Function getSQLPath(ByVal _storage As String, ByVal _name As String, Optional ByVal _isRaster As Boolean = False, Optional ByVal _isOci As Boolean = False, Optional ByVal _isAdoNet As Boolean = False) As String
            ...
            Else
                ...
                If rbSQLite.Checked Then
                    cn = "Sqlite"
                ElseIf _isAdoNet Then
                    cn = "ADONET"
                ElseIf rbGDB.Checked Then
                    cn = GisUtils.GIS_INI_PATH
                Else
                    cn = "ADO"
                End If
            ...
            'finalResult = GisUtils.ConstructParamString(finalResult);
            Return finalResult
        End Function

        Private Sub fillTree(ByVal _name As String, ByVal _ls As TGIS_LayerInfoList, ByVal _type As Integer)
            Dim m As Integer
            Dim i As Integer
            Dim tr As TreeNode
            Dim tc As TreeNode
            ...
            i = 0
            Do While (i <= (_ls.Count - 1))
                tc = tvLayers.SelectedNode.Nodes.Add(_ls.Item(i).Name)
                m = CType(_ls.Item(i).ShapeType, Integer)
                If (m = -1) Then
        ...
        Private Sub btnConnect_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnConnect.Click
            ...
            If canUseStorage("Native") Then
                lv = CType(GisUtils.GisCreateLayer(lname, getSQLPath("Native", lname)), TGIS_LayerVectorSqlAbstract)
            ...
            If canUseStorage("OPENGISBLOB") Then
                lv = CType(GisUtils.GisCreateLayer(lname, getSQLPath("OPENGISBLOB", lname)), TGIS_LayerVectorSqlAbstract)
                If (Not (lv) Is Nothing) Then
                    ls = lv.GetAvailableLayers
            ...
            ElseIf canUseStorage("OPENGISBLOB2") Then
                lv = CType(GisUtils.GisCreateLayer(lname, getSQLPath("OPENGISBLOB2", lname)), TGIS_LayerVectorSqlAbstract)
                If (Not (lv) Is Nothing) Then
                    ls = lv.GetAvailableLayers
            ...
            If canUseStorage("PixelStore2") Then
                la = GisUtils.GisCreateLayer(lname, getSQLPath("PixelStore2", lname, True))
                lp = CType(la, TGIS_LayerPixelSqlAbstract)
            ...
            If (canUseStorage("GEOMEDIA") _
                ...
                lv = CType(GisUtils.GisCreateLayer(lname, getSQLPath("GEOMEDIA", lname)), TGIS_LayerVectorSqlAbstract)
                If (Not (lv) Is Nothing) Then
                    ls = lv.GetAvailableLayers
            ...
            If (canUseStorage("PERSONALGDB") AndAlso canUseDialect("MSJET")) Then
                lv = CType(GisUtils.GisCreateLayer(lname, getSQLPath("PERSONALGDB", lname)), TGIS_LayerVectorSqlAbstract)
                If (Not (lv) Is Nothing) Then
                    ls = lv.GetAvailableLayers
            ...
            If (canUseStorage("POSTGIS") AndAlso canUseDialect("POSTGRESQL")) Then
                lv = CType(GisUtils.GisCreateLayer(lname, getSQLPath("POSTGIS", lname)), TGIS_LayerVectorSqlAbstract)
                If (Not (lv) Is Nothing) Then
                    ls = lv.GetAvailableLayers
            ...
            If ((canUseStorage("SDEBINARY") OrElse canUseStorage("SDEOGCWKB")) _
                ...
                lv = CType(GisUtils.GisCreateLayer(lname, getSQLPath("SDEBINARY", lname)), TGIS_LayerVectorSqlAbstract)
                If (Not (lv) Is Nothing) Then
                    ls = lv.GetAvailableLayers
            ...
            If (canUseStorage("SDERASTER") AndAlso canUseDialect("MSSQL")) Then
                lp = CType(GisUtils.GisCreateLayer(lname, getSQLPath("SDERASTER", lname, True)), TGIS_LayerPixelSqlAbstract)
                If (Not (lp) Is Nothing) Then
                    ls = lp.GetAvailableLayers
            ...
            If (canUseStorage("KATMAI") AndAlso canUseDialect("MSSQL")) Then
                lv = CType(GisUtils.GisCreateLayer(lname, getSQLPath("SDERASTER", lname, False, False, True)), TGIS_LayerVectorSqlAbstract)
                If (Not (lv) Is Nothing) Then
                    ls = lv.GetAvailableLayers
            ...
            If (canUseStorage("SPATIALWARE") AndAlso canUseDialect("MSSQL")) Then
                lv = CType(GisUtils.GisCreateLayer(lname, getSQLPath("SPATIALWARE", lname)), TGIS_LayerVectorSqlAbstract)
                If (Not (lv) Is Nothing) Then
                    ls = lv.GetAvailableLayers
            ...
            If (canUseStorage("ORACLESPATIAL") AndAlso canUseDialect("ORACLE")) Then
                lv = CType(GisUtils.GisCreateLayer(lname, getSQLPath("ORACLESPATIAL", lname, False, True)), TGIS_LayerVectorSqlAbstract)
                If (Not (lv) Is Nothing) Then
                    ls = lv.GetAvailableLayers
            ...
            If (canUseStorage("ORACLEGEORASTER") AndAlso canUseDialect("ORACLE")) Then
                lp = CType(GisUtils.GisCreateLayer(lname, getSQLPath("ORACLEGEORASTER", lname, True, True)), TGIS_LayerPixelSqlAbstract)
                If (Not (lp) Is Nothing) Then
                    ls = lp.GetAvailableLayers
            ...
            If (canUseStorage("DB2GSE") AndAlso canUseDialect("DB2")) Then
                lv = CType(GisUtils.GisCreateLayer(lname, getSQLPath("DB2GSE", lname)), TGIS_LayerVectorSqlAbstract)
                If (Not (lv) Is Nothing) Then
                    ls = lv.GetAvailableLayers
            ...
            If (canUseStorage("IFXSDB") AndAlso canUseDialect("INFORMIX")) Then
                lv = CType(GisUtils.GisCreateLayer(lname, getSQLPath("IFXSDB", lname)), TGIS_LayerVectorSqlAbstract)
                If (Not (lv) Is Nothing) Then
                    ls = lv.GetAvailableLayers
            ...
            If (canUseStorage("ORACLESPATIAL_PC") AndAlso canUseDialect("ORACLE")) Then
                lv = CType(GisUtils.GisCreateLayer(lname, getSQLPath("ORACLESPATIAL_PC", lname, False, True)), TGIS_LayerVectorSqlAbstract)
                If (Not (lv) Is Nothing) Then
                    ls = lv.GetAvailableLayers
            ...
            If (canUseStorage("ORACLESPATIAL_TIN") AndAlso canUseDialect("ORACLE")) Then
                lv = CType(GisUtils.GisCreateLayer(lname, getSQLPath("ORACLESPATIAL_TIN", lname, False, True)), TGIS_LayerVectorSqlAbstract)
                If (Not (lv) Is Nothing) Then
                    ls = lv.GetAvailableLayers
            ...
            If (canUseStorage("GEOMEDIAMSSPATIAL") AndAlso canUseDialect("MSSQL")) Then
                lv = CType(GisUtils.GisCreateLayer(lname, getSQLPath("GEOMEDIAMSSPATIAL", lname, False, False, True)), TGIS_LayerVectorSqlAbstract)
                If (Not (lv) Is Nothing) Then
                    ls = lv.GetAvailableLayers
            ...
            If (canUseStorage("GEOMEDIAORACLESPATIAL") AndAlso canUseDialect("ORACLE")) Then
                lv = CType(GisUtils.GisCreateLayer(lname, getSQLPath("GEOMEDIAORACLESPATIAL", lname, False, True)), TGIS_LayerVectorSqlAbstract)
                If (Not (lv) Is Nothing) Then
                    ls = lv.GetAvailableLayers
            ...
            If (canUseStorage("ANYWHERESPATIAL") AndAlso canUseDialect("SYBASE")) Then
                lv = CType(GisUtils.GisCreateLayer(lname, getSQLPath("ANYWHERESPATIAL", lname)), TGIS_LayerVectorSqlAbstract)
                If (Not (lv) Is Nothing) Then
                    ls = lv.GetAvailableLayers
            ...
            If ((Not (tvLayers.SelectedNode) Is Nothing) _
                ...
                If (TypeOf ll Is TGIS_LayerVectorSqlAbstract) Then
                    ...
                    Do While (i _
                        ...
                        rtbLayerParams.AppendText((CType(ll, TGIS_LayerVectorSqlAbstract).SQLParametersEx.Strings(i) + " = "))
                        lst.Add((CType(ll, TGIS_LayerVectorSqlAbstract).SQLParametersEx.Strings(i) + " = "))
                        i = (i + 1)
                    Loop
                ...
                ElseIf (TypeOf ll Is TGIS_LayerPixelSqlAbstract) Then
                    ...
                    Do While (i _
                        ...
                        rtbLayerParams.AppendText((CType(ll, TGIS_LayerPixelSqlAbstract).SQLParametersEx.Strings(i) + " = "))
                        lst.Add((CType(ll, TGIS_LayerPixelSqlAbstract).SQLParametersEx.Strings(i) + " = "))
                        i = (i + 1)
                    Loop
        ...
        Private Function getLayer(ByVal _name As String, ByVal _type As Integer) As TGIS_Layer
            Select Case (_type)
                Case LAYERSQL_NATIVE
                    Return GisUtils.GisCreateLayer(_name, getSQLPath("Native", _name))
                Case LAYERSQL_OPENGISBLOB
                    Return GisUtils.GisCreateLayer(_name, getSQLPath("OpenGisBlob", _name))
                Case LAYERSQL_OPENGISBLOB2
                    Return GisUtils.GisCreateLayer(_name, getSQLPath("OpenGisBlob2", _name))
                Case LAYERSQL_GEOMEDIA
                    Return GisUtils.GisCreateLayer(_name, getSQLPath("Geomedia", _name))
                Case LAYERSQL_POSTGIS
                    Return GisUtils.GisCreateLayer(_name, getSQLPath("PostGis", _name))
                Case LAYERSQL_PERSONALGDB
                    Return GisUtils.GisCreateLayer(_name, getSQLPath("PersonalGdb", _name))
                Case LAYERSQL_SDEBINARY
                    Return GisUtils.GisCreateLayer(_name, getSQLPath("SdeBinary", _name))
                Case LAYERSQL_PIXELSTORE2
                    Return GisUtils.GisCreateLayer(_name, getSQLPath("PixelStore2", _name, True))
                Case LAYERSQL_KATMAI
                    Return GisUtils.GisCreateLayer(_name, getSQLPath("Katmai", _name, False, False, True))
                Case LAYERSQL_ORACLESPATIAL
                    Return GisUtils.GisCreateLayer(_name, getSQLPath("OracleSpatial", _name, False, True))
                Case LAYERSQL_SDERASTER
                    Return GisUtils.GisCreateLayer(_name, getSQLPath("SdeRaster", _name, True))
                Case LAYERSQL_ORACLEGEORASTER
                    Return GisUtils.GisCreateLayer(_name, getSQLPath("OracleGeoraster", _name, True, True))
                Case LAYERSQL_SPATIALWARE
                    Return GisUtils.GisCreateLayer(_name, getSQLPath("SPATIALWARE", _name))
                Case LAYERSQL_DB2GSE
                    Return GisUtils.GisCreateLayer(_name, getSQLPath("DB2GSE", _name))
                Case LAYERSQL_IFXSDB
                    Return GisUtils.GisCreateLayer(_name, getSQLPath("IFXSDB", _name))
                Case LAYERSQL_FGDB
                    Return GisUtils.GisCreateLayer(_name, getSQLPath("FILEGDB", _name))
                Case LAYERSQL_ORACLESPATIALPC
                    Return GisUtils.GisCreateLayer(_name, getSQLPath("ORACLESPATIAL_PC", _name, False, True))
                Case LAYERSQL_ORACLESPATIALTIN
                    Return GisUtils.GisCreateLayer(_name, getSQLPath("ORACLESPATIAL_TIN", _name, False, True))
                Case LAYERSQL_GEOMEDIA_MSSQL
                    Return GisUtils.GisCreateLayer(_name, getSQLPath("GEOMEDIAMSSPATIAL", _name, False, False, True))
                Case LAYERSQL_GEOMEDIA_SDO
                    Return GisUtils.GisCreateLayer(_name, getSQLPath("GEOMEDIAORACLESPATIAL", _name, False, True))
                Case LAYERSQL_ANYWHERE_SPATIAL
                    Return GisUtils.GisCreateLayer(_name, getSQLPath("ANYWHERESPATIAL", _name))
                Case LAYERSQL_OGR
                    Return GisUtils.GisCreateLayer(_name, getSQLPath("OGR", _name))
                Case Else
                    Return Nothing
            End Select
        ...
        Private Sub btnCreateConf_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnCreateConf.Click
            ...
            If ((Not (tvLayers.SelectedNode) Is Nothing) _
                ...
                ini = (New TGIS_ConfigFactory).CreateConfig(Nothing, ttklspath)
                ini.Section = GisUtils.GIS_INI_LAYER_HEADER
                ...
                If (storage = "KATMAI") Then
                    ini.WriteString(ini.Section, "CONNECTOR_ADONET", prepareADONETCS)
                ...
                Else
                    If rbSQLite.Checked Then
                        ini.WriteString(ini.Section, "SQLITE", GisUtils.GisPathRelative(GisUtils.GisFilePath(ttklspath), cbxConnString.Text))
                    End If

                    If rbGDB.Checked Then
                        ini.WriteString(ini.Section, GisUtils.GIS_INI_PATH, GisUtils.GisPathRelative(GisUtils.GisFilePath(ttklspath), cbxConnString.Text))
                    Else
