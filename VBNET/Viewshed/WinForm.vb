...
Namespace AddLayer
    ...
    ''' CreateShape_2 and AddShape_2 are COM overload aliases.
    ''' </summary>
...
#Region "Windows Form Designer generated code"
        ...
        Private Sub InitializeComponent()
            ...
            Me.GIS.Size = New System.Drawing.Size(577, 400)
            Me.GIS.TabIndex = 0
            '
            'btnFullExtent
            '
            ...
            Me.lblHint.Size = New System.Drawing.Size(580, 17)
...
#End Region
        ...
        Private Sub makeViewshedRamp()
            ...
            If rbtnViewshedBinary.Checked Then
                lViewshed.GenerateRamp(
                  (New TGIS_Color).FromARGB(127, 0, 255, 0),
                  (New TGIS_Color).None,
                  (New TGIS_Color).FromARGB(127, 0, 255, 0),
            ...
            ElseIf rbtnViewshedFreq.Checked Then
                lViewshed.GenerateRamp(
                  (New TGIS_Color).FromARGB(127, 255, 0, 0),
                  (New TGIS_Color).None,
                  (New TGIS_Color).FromARGB(127, 0, 255, 0),
                  0, 0,
                  lViewshed.MaxHeight, False,
        ...
        Private Sub WinForm_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
            rbtnAddObserver.Checked = True
            rbtnViewshedBinary.Checked = True
            ...
            lObservers.Params.Marker.Symbol =
              GisUtils.SymbolList.Prepare("LIBSVG:std:TowerCommunication01")
            lObservers.Params.Marker.Color = (New TGIS_Color).White
            lObservers.Params.Marker.OutlineColor = (New TGIS_Color).White
        ...
        Private Sub GIS_MouseDown(sender As Object, e As AxTatukGIS_XDK11.ITGIS_ViewerWndEvents_MouseDownEvent) Handles GIS.MouseDownEvent
            ...
            If GIS.Mode = TGIS_ViewerMode.UserDefined Then
                Try
                    elev = CType(Double.Parse(edtObserverElevation.Text), Single)
                ...
                Try
                    ...
                    pt = GIS.ScreenToMap(GisUtils.Point(e.X, e.Y))
                    If Not GisUtils.GisIsPointInsideExtent(pt, lTerrain.Extent) Then
                        Return
                    End If

                    '' add observer to the observer layer
                    shp = lObservers.CreateShape_2(TGIS_ShapeType.Point, TGIS_DimensionType.XY)
                    shp.AddPart()
                    shp.AddPoint(pt)
        ...
        Private Sub GIS_MouseMove(sender As Object, e As AxTatukGIS_XDK11.ITGIS_ViewerWndEvents_MouseMoveEvent) Handles GIS.MouseMoveEvent
            Dim ptg As TGIS_Point
            Dim cl As New TGIS_Color
            Dim vals As New TGIS_DoubleArray
            ...
            If lViewshed IsNot Nothing Then
                If lViewshed.Locate(ptg, cl, vals, transp) Then
                    If vals.Value(0) <> lViewshed.NoDataValue Then
                        str = str + "Frequency: " + vals.Value(0).ToString
                    End If
                End If
            End If
            If lAGL IsNot Nothing Then
                If lAGL.Locate(ptg, cl, vals, transp) Then
                    If vals.Value(0) <> lAGL.NoDataValue Then
                        str = str + "Above-Ground-Level: " + vals.Value(0).ToString
                    End If
                End If
            End If
