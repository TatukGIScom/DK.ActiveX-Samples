...
Namespace DemOperations
    ...
    Public Class WinForm
        ...
        Private ControlLegend As AxTGIS_ControlLegend
...
#Region "Windows Form Designer generated code"
        ...
        Private Sub InitializeComponent()
            ...
            Me.ControlLegend.Location = New System.Drawing.Point(767, 30)
            ...
            Me.GIS.Size = New System.Drawing.Size(553, 655)
            Me.GIS.TabIndex = 7
            '
            'WinForm
            '
...
#End Region
        ...
        Private Sub ChangeDem(_layer As TBaseObject, _extent As TGIS_Extent, _source As TGIS_GridArray, ByRef _output As TGIS_GridArray, _width As Integer, _height As Integer, ByRef _minz As Single, ByRef _maxz As Single, ByRef _result As Boolean)
            ...
            For i As Integer = 2 To _height - 2
                ...
                For j As Integer = 1 To _width - 2
                    AWindow(0) = _source.Value(l1, j - 1)
                    AWindow(1) = _source.Value(l1, j)
                    AWindow(2) = _source.Value(l1, j + 1)
                    AWindow(3) = _source.Value(l2, j - 1)
                    AWindow(4) = _source.Value(l2, j)
                    AWindow(5) = _source.Value(l2, j + 1)
                    AWindow(6) = _source.Value(l3, j - 1)
                    AWindow(7) = _source.Value(l3, j)
                    AWindow(8) = _source.Value(l3, j + 1)
                    ...
                    End If
                    If (_source.Value(l1, j) <> inodata) Then
        ...
        Private Sub cbxCustomGrid_CheckedChanged(sender As Object, e As EventArgs) Handles cbxCustomGrid.CheckedChanged
            ...
            If (EventLayer Is Nothing) Then Return

            If (cbxCustomGrid.Checked) Then
                EventLayer.Params.Pixel.AltitudeMapZones.Clear()
                EventLayer.Params.Pixel.GridShadow = False
                AddHandler EventLayer.GridOperationEvent, AddressOf ChangeDem
            Else
                EventLayer.Params.Pixel.GridShadow = True
            End If
        ...
        Private Sub btnRun_Click(sender As Object, e As EventArgs) Handles btnRun.Click
            ...
            Select Case cbOperations.SelectedIndex
                'HillShade
                Case 0
                    dop = New TGIS_DemOperationHillShade()
                    dop.Create_(Double.Parse(tbZFactor.Text),
                ...
                Case 1
                    ...
                    End Select
                    dop = New TGIS_DemOperationSlope
                    dop.Create_(sm, Double.Parse(tbScale.Text))
                'SlopeHydro
                Case 2
                    Select Case cbMode.SelectedIndex
                        Case 0 : sm = TGIS_DemSlopeMode.Degrees
                        ...
                        Case Else : sm = TGIS_DemSlopeMode.Degrees
                    End Select
                    dop = New TGIS_DemOperationSlopeHydro
                    dop.Create_(sm, Double.Parse(tbScale.Text))
                'Aspect
                Case 3 : dop = New TGIS_DemOperationAspect
                    dop.Create_(cbxAngle.Checked)
                ' TRI
                ...
                Case 7
                    ...
                    End Select
                    dop = New TGIS_DemOperationTotalCurvature
                    dop.Create_(cm)
                'MatrixGain
            ...
            GIS.Add(ld)
            dem.Process(lp, lp.Extent, ld, dop, Nothing)
            GIS.InvalidateWholeMap()
        End Sub
        ...
        Private Sub GIS_BusyEvent(sender As Object, e As ITGIS_ViewerWndEvents_BusyEventEvent) Handles GIS.BusyEvent
            ...
            Else
                ...
                pbProgress.Value = CType(e.Pos / e.End * 100, Integer)
            End If
            pbProgress.Update()
